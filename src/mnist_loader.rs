...
pub struct MnistData {
    pub train_images: Array4<f64>,
    pub train_labels: Array4<f64>,
    pub test_images: Array4<f64>,
    pub test_labels: Array4<f64>,
}
...
fn process_images(data: Vec<u8>, size: usize) -> Array4<f64> {
    let data_f64: Vec<f64> = data.into_iter().map(|x| x as f64 / 255.0).collect();
    Array4::from_shape_vec((size, 1, 28, 28), data_f64)
        .expect("Error reshaping images")
}

fn process_labels(labels: Vec<u8>, size: usize) -> Array4<f64> {
    let mut one_hot = Array4::<f64>::zeros((size, 1, 1, 10));
